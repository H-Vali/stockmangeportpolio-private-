...
- 서버로 올림: 투자자 추가/수정/삭제, 입출금, 매수/매도 등록·수정·삭제
...
탭을 여러 개 켜두면 폴링 주기를 더 늘리세요(`src/net/stocks.js`의 `stockBackoff`/
`indexBackoff` 최대 대기 시간, `src/net/scheduler.js`의 `15000`/`30000`).
...
**보유주식과 지수는 서로 다른 주기로 폴링합니다** — 지수는 손익 계산에 안 쓰이는
참고용이라 예산을 덜 쓰게 하고, 그만큼을 실제 평가금액에 반영되는 보유주식 쪽에
더 씁니다.

| 대상 | 주기 | 종목 수 | 분당 요청 |
|---|---|---|---|
| 보유주식 | 15초 | 보유한 만큼 (N) | `N × 4` |
| 지수 | 30초 | 3(나스닥·S&P500·러셀2000) | `6` |

탭 1개 기준으로 보유 미국주식이 10종목이면 분당 `10×4 + 6 = 46`회로 한도(60) 안에
들어오지만, 종목이 13개를 넘어가면 이론상 한도를 넘길 수 있습니다. 실제로 넘겨도
아래 백오프가 자동으로 회복시키므로 앱이 죽지는 않고, 시세가 잠깐 갱신을 건너뛰는
정도입니다. 종목이 아주 많다면 `src/net/scheduler.js`의 `15000`(보유주식 주기)을
늘리세요.

**"여러 PC에서 동시 접속"이 이 앱의 정상 사용 패턴이라는 점**도 감안해야 합니다 —
탭을 여러 개 열어두면 그만큼 곱해집니다. 세 가지로 대응해 뒀습니다.
...
- **연속 실패 시 자동 백오프** (`src/net/stocks.js`) — 429가 나면 보유주식·지수
  각각 독립적으로 다음 몇 번의 폴링 주기(최대 약 2분)를 건너뛰어 한도가 풀릴
  시간을 줍니다. 한쪽이 막혀도 다른 쪽까지 같이 멈추지 않습니다. 성공하면 즉시
  원래 주기로 복귀합니다.
- **실패 응답은 캐시하지 않음** (`functions/quote.js`) — 429를 60초씩 캐시해
  버리면 한도가 풀린 뒤에도 낡은 실패 응답이 계속 나갑니다. 성공 응답만 캐시합니다.

코인은 이 예산과 완전히 무관합니다(`src/net/crypto.js`) — Finnhub를 거치지 않고
브라우저가 Binance WebSocket(해외, 실시간 푸시)과 빗썸 REST(국내, 5초 폴링)를
직접 호출하고, 15초마다 전체 재동기화까지 독자적으로 돕니다. 보유주식·지수 쪽이
429로 잠깐 멈춰도 코인 시세는 영향을 받지 않습니다.
