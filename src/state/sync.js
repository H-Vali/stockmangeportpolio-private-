...
export async function pushStateToServer() {
  ...
  try {
    ...
    if (response.status === 409) {
      ...
      logger.warn("sync", `개정 충돌: 로컬 ${knownServerRev} / 서버 ${detail?.rev} - 최신 수정시각 기준으로 자동 정리`);
      conflicted = true;
      syncPushInFlight = false;
      await autoResolveConflict();
      return;
    }
...
// 다른 기기가 먼저 저장해 rev 충돌(409)이 났을 때, 버튼 없이 자동으로 정리한다.
// 두 기기의 state.updatedAt(마지막 편집 시각)을 비교해 더 최근에 편집된 쪽을 그대로 채택한다.
// 사람이 매번 "내 걸 쓸지 서버 걸 쓸지" 고르게 하지 않는 대신, 두 기기 시계가 크게
// 어긋나 있으면 오판할 수 있다는 트레이드오프를 감수한다(개인 용도라 실사용 빈도 낮음).
async function autoResolveConflict() {
  try {
    const response = await fetch("/state", { headers: { "Authorization": `Bearer ${getSyncToken()}` } });
    if (!response.ok) {
      setSyncStatus("error", `동기화 충돌 정리 실패 (${response.status})`);
      return;
    }
    const data = await response.json();
    if (data?.rev != null) knownServerRev = Number(data.rev);

    const serverUpdatedAt = data?.state?.updatedAt ? Date.parse(data.state.updatedAt) : 0;
    const localUpdatedAt = state.updatedAt ? Date.parse(state.updatedAt) : 0;

    if (localUpdatedAt > serverUpdatedAt) {
      // 이 기기 편집이 더 최근 -> 강제로 밀어올린다.
      conflicted = false;
      await pushStateToServer();
      logger.info("sync", "충돌 자동 정리: 이 기기(최근 편집)가 서버에 반영됨");
      return;
    }

    // 서버 쪽이 더 최근(또는 동시) -> 이 기기를 서버 값으로 맞춘다.
    const errors = data?.state ? validateImportState(data.state) : ["state 없음"];
    if (errors.length) {
      logger.warn("sync", `충돌 자동 정리 중 서버 데이터 검증 실패: ${errors[0]}`);
    ...
    clearDirty();
    persistLocal();
    setSyncStatus("synced", "다른 기기의 최신 작업 내용으로 자동 갱신했습니다.");
    render();
    logger.info("sync", "충돌 자동 정리: 서버(다른 기기, 최근 편집)를 이 기기에 반영함");
  } catch (error) {
    logger.error("sync", "충돌 자동 정리 실패", error);
    setSyncStatus("offline", "서버에 연결할 수 없어 로컬 데이터를 유지합니다.");
  }
}
