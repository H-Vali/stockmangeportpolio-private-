...
test("buildDividendForecast: 분기 배당 이력이면 다음 분기 간격으로 예측을 이어간다", () => {
  ...
  const forecast = buildDividendForecast(records, { now, horizonMonths: 12 });
...
test("buildDividendForecast: 이력이 1건뿐이면 간격을 알 수 없어 그 값만 반환한다 (미래 지급일인 경우)", () => {
  ...
  const forecast = buildDividendForecast(records, { now: new Date("2026-01-01T00:00:00Z"), horizonMonths: 12 });
...
test("buildDividendForecast: 예측 범위(horizonMonths)를 넘어서는 항목은 자르낸다", () => {
  const records = normalizeDividendRecords([
    rec({ date: "2025-01-15", amount: 1 }),
  ...
  const forecast = buildDividendForecast(records, { now: new Date("2026-01-01T00:00:00Z"), horizonMonths: 12 });
  forecast.forEach((p) => assert.ok(p.payDate <= "2027-01-01"));
});
