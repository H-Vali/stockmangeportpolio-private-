...
test("예수금 = 입금 - 출금 - 매수금액 + 매도대금", () => {
  useState({
    trades: [
      buy({ quantity: 10, price: 80, fx: 1300 }), // 1,040,000 매수
      buy({ date: "2026-02-01", side: "sell", quantity: 5, price: 90, fx: 1350 }) // 607,500 매도
    ],
    cashflows: [
      { id: "c1", ownerId: OWNER, date: "2026-01-01", type: "deposit", amount: 2_000_000 },
      { id: "c2", ownerId: OWNER, date: "2026-01-05", type: "withdraw", amount: 300_000 }
    ],
    assetCatalog: SCHD_AT(90, 1350)
  });

  assert.equal(netCashflow(OWNER), 1_700_000);
  assert.equal(cashBalance(OWNER), 1_700_000 - 1_040_000 + 607_500);
});

test("요약: 평가금액 = 보유평가 + 예수금, 평가손익 = 평가금액 - 투자원금", () => {
  useState({
  ...
  assert.equal(summary.cash, 2_000_000 - 1_040_000);
...
test("자산배분: 유형별 합계와 예수금이 모두 잡힌다", () => {
  ...
  assert.equal(cash.value, 960_000);
