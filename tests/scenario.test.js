...
//
// 시나리오
//   김철수: 2,000만 입금 -> SCHD 100주 @$80 (환율 1300) -> 50주 @$60 (환율 1400)
//           -> 30주 @$95 매도 (환율 1400). 현재가 $90, 현재환율 1400
...
import { cashBalance, replayHoldings, summarize } from "../src/domain/portfolio.js";
...
const A_CASH = 20_000_000 - 10_400_000 - 4_200_000 + 30 * 95 * 1400; // 9,390,000
...
test("투자자별 평가금액과 평가손익", () => {
  const a = summarize(A);
  near(a.principal, 20_000_000);
  near(a.totalValue, A_HOLDINGS + A_CASH); // 24,510,000
  near(a.profit, 4_510_000);
...
test("전체 합계는 투자자 합과 같다", () => {
  const all = summarize();
  near(all.totalValue, 35_010_000);
  near(all.principal, 30_000_000);
  near(all.profit, 5_010_000);
...
test("매도 거래를 지우면 수량과 예수금이 매도 전으로 돌아간다", () => {
  ...
  near(cashBalance(A), 20_000_000 - 10_400_000 - 4_200_000);
});
