...
| `src/state/sync.js` | Worker `/state` 동기화. rev 기반 충돌 감지 |
...
호출 지점은 환율 갱신(`net/fx.js`), 거래 등록·수정(`domain/actions.js`),
수동 환율 입력(`ui/events.js`) 세 군데입니다.
...
지금은 Worker 가 KV 메타데이터에 `rev` 를 함께 보관합니다.
